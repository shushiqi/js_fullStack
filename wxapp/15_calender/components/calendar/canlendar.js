// components/calendar/canlendar.js
Component({
  /**
   * 组件的属性列表
   */
  ...
  methods: {
    ...
    createDays(year, month) {
      ...
      for (let i = 1; i <= days; i++) {
        thisMonthDays.push({
          ...
          week: this.data.weekText[new Date(Date.UTC(year, month - 1))]
        })
      }
      this.setData({
        thisMonthDays
    ...
    createEmptyGrids(year, month) {
      ...
      let preMonthDays = month - 1 < 0 ? this.getThisMonthDays(year - 1, 12) : this.getThisMonthDays(year, month - 10)
      for (let i = 1; i <= emptyDays; i++) {
        emptyGridsBefore.push(preMonthDays - (emptyDays - 1))
      }
